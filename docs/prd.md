...
**## 2. Product design system

### 2.1 Visual direction

The product UI should use an **organic neo-brutalist SaaS** visual language with:

- Asymmetric cutout sections and notched cards.
- Irregular rounded containers rather than uniform card grids.
- Layered panels and bento-style composition.
- Controlled gradients and soft depth against a clean SaaS structure.
- Strong visual hierarchy, generous spacing, and deliberate asymmetry.
- Crisp borders and tactile panel relationships without making the interface feel heavy.
- Responsive layouts that preserve the intended composition across mobile, tablet, and desktop.

The attached `docs/palette.svg` is the source of truth for the project's color palette. Do not invent a separate primary palette. Derive UI colors, gradients, borders, surfaces, accents, states, and supporting tones from that palette.

### 2.2 Typography

Use Google Fonts with the following typography system:

- **Headings:** Montserrat.
- **Body:** Montserrat or Lato.
- **Accent script:** Great Vibes, used sparingly for subtle brand personality.
- **Style:** Cursive accent + clean sans serif.

Great Vibes should be reserved for small decorative or brand-accent moments, never for long-form body copy, technical content, navigation, forms, or critical UI labels.
...
Do not use Tailwind CSS, CSS-in-JS, or a component library as the primary styling system. Build reusable React components and maintain a structured CSS architecture using the project's design tokens and the palette source of truth.
...
  API["React + Vite on Vercel\<br/>REST + x402 + MCP"] --> CORE["packages/core\<br/>pure verify + receipt"]
...
\- **\*\*FR-0.6\*\*** CI runs install, typecheck, and lint on every push; smart-contract tests also run in CI.
...
### 6.1 Contract-only testing policy

Tests are required only for the smart contracts. No frontend, API, SDK, CLI, MCP, database, or UI test suites are required for v1.

### 6.2 Smart contract tests

Use Foundry for contract testing.

- Unit tests for all public/external contract behavior.
- Fuzz tests for inputs and state transitions where useful.
- Access-control and authorization tests where applicable.
- Immutability tests confirming there is no upgrade/admin path.
- Event emission tests.
- Anchor, revocation, and issuer-registry state tests.
- Revert and edge-case tests.
- Fork/integration tests only where they directly validate contract behavior against Base Sepolia.

### 6.3 Definition of done

- All smart contract tests pass locally and in CI.
- Contract behavior matches the requirements in Phase 2.
- No frontend or application-level automated test suite is required.
- Manual verification may be used for frontend and service acceptance criteria.
